...
fig_weather.add_trace(go.Bar(
    ...
    textposition='outside',
    hovertemplate='<b>%{x}</b><br>Energy: %{y:.1f} kWh<br>Revenue: ₹%{customdata:.0f}<extra></extra>',
...
comparison_display.columns = ['Weather', 'Total Energy (kWh)', 'Daily Avg (kWh)', 'Peak Power (kW)', 'Revenue (₹)']
...
with health_col2:
    ...
    if metrics['capacity_factor'] > 18:
        health_checks.append(("✅", "Capacity Factor", f"{metrics['capacity_factor']:.1f}%", "Excellent"))
    elif metrics['capacity_factor'] > 15:
        health_checks.append(("⚠", "Capacity Factor", f"{metrics['capacity_factor']:.1f}%", "Good"))
    else:
        health_checks.append(("❌", "Capacity Factor", f"{metrics['capacity_factor']:.1f}%", "Low"))

    # Peak Power
    peak_achievement = (metrics['peak_power'] / SYSTEM_CONFIG['panel_capacity_kw']) * 100
    if peak_achievement > 90:
        health_checks.append(("✅", "Peak Power", f"{peak_achievement:.0f}%", "Optimal"))
    elif peak_achievement > 75:
        health_checks.append(("⚠", "Peak Power", f"{peak_achievement:.0f}%", "Acceptable"))
    else:
        health_checks.append(("❌", "Peak Power", f"{peak_achievement:.0f}%", "Degraded"))

    # Temperature
    max_panel_temp = forecast_df['panel_temp'].max()
    if max_panel_temp < 60:
        health_checks.append(("✅", "Temperature", f"{max_panel_temp:.1f}°C", "Normal"))
    elif max_panel_temp < 70:
        health_checks.append(("⚠", "Temperature", f"{max_panel_temp:.1f}°C", "Elevated"))
    else:
        health_checks.append(("❌", "Temperature", f"{max_panel_temp:.1f}°C", "Critical"))

    # Consistency
    daily_std = np.std(metrics['daily_energies'])
    daily_cv = (daily_std / metrics['daily_avg']) * 100 if metrics['daily_avg'] > 0 else 0
    if daily_cv < 20:
        health_checks.append(("✅", "Consistency", f"{daily_cv:.1f}% CV", "Stable"))
    elif daily_cv < 30:
        health_checks.append(("⚠", "Consistency", f"{daily_cv:.1f}% CV", "Variable"))
    else:
        health_checks.append(("❌", "Consistency", f"{daily_cv:.1f}% CV", "Unstable"))
