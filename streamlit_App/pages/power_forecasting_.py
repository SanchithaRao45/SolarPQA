...
# Standards-based RUL Endpoints (For systems below 1kV)
# VTHD Limit: IEEE 519 Standard for VTHD <= 69 kV (which covers < 1kV)
VTHD_CRITICAL_THRESHOLD = 5.0
# ITHD Limit: Conservative limit for residential inverter component health (often stricter than utility limits)
ITHD_CRITICAL_THRESHOLD = 15.0
# Power Factor Limit: Common utility penalty threshold
...
ACTION_MAPPING = {
    "System Resonance": "EMERGENCY: Install a single-phase Active Harmonic Filter (AHF) or upgrade passive components to prevent equipment damage and meet safety standards.",
    "Harmonic Distortion": "Inspect or replace residential load-side filters (e.g., in variable speed drives). High Current THD is stressing low-voltage components.",
    "Low Power Factor (Reactive Load)": "Check small capacitor banks or automatic PFC modules. Adjust inverter controls to inject reactive power.",
...
def run_predictive_analysis(vthd_7d, ithd_7d, pf_14d, vthd_rate, ithd_rate, pf_rate):
    ...
    if vthd_rate > 0:
        ...
        if rul_vthd is not None and rul_vthd <= 60: # Warning if RUL is less than 2 months
            ...
            alerts.append({
                'Status': status,
                'Type': 'VTHD Compliance',
                'Prognosis': f"VTHD RUL: *{rul_vthd:.1f} days*. Voltage distortion is trending towards the safety limit of {VTHD_CRITICAL_THRESHOLD}%. Rate: {vthd_rate:.4f} %/day."
            })
...
def get_rul_summary(pm_report):
    """Generates a user-friendly RUL summary from PM alerts."""
    critical_alerts = [a for a in pm_report if a['Status'] == 'CRITICAL']
    warning_alerts = [a for a in pm_report if a['Status'] == 'WARNING']
    
    # 1. Prioritize CRITICAL alerts
    if critical_alerts:
        alert = critical_alerts[0] 
        prognosis_text = alert['Prognosis'].replace('*', '')
        
        try:
            rul_days_str = prognosis_text.split(':')[1].split('days')[0].strip()
            rul_days = float(rul_days_str)
            
            if rul_days < 7:
                severity = "IMMINENT FAILURE"
                timeline_desc = "less than **1 week**"
                color = "#dc3545" # Red
            elif rul_days < 30:
                severity = "HIGH RISK"
                timeline_desc = f"in **{int(rul_days)} days**"
                color = "#ffc107" # Yellow
            else:
            ...
            return {
                ...
                <p style='margin: 0; font-size: 1.1rem;'>
                    <span style='font-weight: bold; color: {color};'>{severity}:</span> 
                    The '{alert['Type']}' condition is trending towards critical failure or non-compliance 
                    {timeline_desc} based on the current degradation rate.
                </p>
                <p style='margin: 0.5rem 0 0 0; font-size: 0.9rem; opacity: 0.8;'>
                    *RUL Calculation: {prognosis_text}*
                </p>
                """,
                'is_alert': True
            }
            ...
            return {
                ...
                'prognosis_html': f"<p style='color: #dc3545;'>A critical trend was detected for {alert['Type']}, but the RUL calculation is uncertain. Immediate investigation required.</p>",
                'is_alert': True
            }

    # 2. Handle WARNING alerts
    elif warning_alerts:
        alert = warning_alerts[0]
        prognosis_text = alert['Prognosis'].replace('*', '')
        return {
            'severity': "LONG-TERM WARNING",
            'prognosis_html': f"""
            <p style='margin: 0; font-size: 1.1rem; color: #007bff;'>
                <span style='font-weight: bold;'>PREVENTATIVE MONITORING:</span> 
                A slow degradation trend for '{alert['Type']}' is observed. Threshold violation is expected in several months.
            </p>
            <p style='margin: 0.5rem 0 0 0; font-size: 0.9rem; opacity: 0.8;'>
                *RUL Calculation: {prognosis_text}*
            </p>
    ...
    # 3. Normal
        ...
        return {
            'severity': "SYSTEM STABLE",
            'prognosis_html': "<p style='color: #28a745;'>All system parameters and trends are stable and fully compliant with IEEE 519 standards.</p>",
            'is_alert': False
        }
...
def get_unified_analysis(input_data):
    ...
    rul_summary = get_rul_summary(pm_report)
...
st.info(f"""
    **Standard Context:** This monitoring system applies to low-voltage systems (below $1 \text{kV}$), primarily focusing on:
    1.  Meeting the **IEEE 519 VTHD limit of $5.0 \%$** for safety and compliance.
    2.  Preventing premature component failure using a conservative **ITHD component limit of $15.0 \%$**.
""")
st.markdown("---")
...
# --- VIII. SYSTEM DATA OVERVIEW (UNCHANGED) ---
...
# --- IX. ROOT CAUSE ANALYSIS (MODIFIED) ---
st.markdown('<div class="section-header">Root Cause Analysis & Predictive Prognosis</div>', unsafe_allow_html=True)
...
rul_summary = report['rul_summary']
...
with rca_main:
    ...
    if diagnosis == 'Normal Operation':
        st.markdown(f'## <span style="color: #28a745;">System Status: {diagnosis}</span>', unsafe_allow_html=True)
    elif priority == 'Urgent' or priority == 'High':
        st.markdown(f'## <span style="color: #dc3545;">IMMEDIATE FAULT: {diagnosis}</span>', unsafe_allow_html=True)
    else:
        st.markdown(f'## <span style="color: #ffc107;">FAULT: {diagnosis}</span>', unsafe_allow_html=True)

    st.markdown("---")
    
    # 2. RUL Prognosis (User-friendly format)
    st.markdown("##### ⏳ Predictive Prognosis")
    st.markdown(f"<div class='rul-prognosis-box'>{rul_summary['prognosis_html']}</div>", unsafe_allow_html=True)
    
    st.markdown("---")

    # 3. Recommended Solution/Action
...
with rca_side:
    ...
    st.markdown("---")
    st.markdown("##### Key Standard Limits")
    st.metric(label=f"VTHD Max Limit (IEEE 519)", value=f"{VTHD_CRITICAL_THRESHOLD}%")
    st.metric(label=f"ITHD Component Health Limit", value=f"{ITHD_CRITICAL_THRESHOLD}%")

# --- X. PREDICTIVE MAINTENANCE (SLIGHTLY MODIFIED) ---
...
if has_alerts:
    st.markdown("##### Detailed RUL Trend Readings:")
    for alert in alerts_to_display:
        if alert['Status'] == 'CRITICAL':
            st.error(f"CRITICAL: {alert['Type']} - Expected failure/non-compliance soon.")
        elif alert['Status'] == 'WARNING':
            st.warning(f"WARNING: {alert['Type']} - Long-term trend detected.")
